...
if($redirect_url==''){
$redirect_url='https://upi.cryptrx.in/';    
}

 $txn_data = file_get_contents('https://upi.cryptrx.in/phnpe/user_txn.php?no='.$user_token.''); 
...
$qr_code_url = "https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=" . urlencode($orders);  //google chartapi
...
function upiCountdown(elm, minute, second, url) {
    ...
    function startTimer() {
        ...
        if(m < 0){
            ...
            window.location.href = "https://upi.cryptrx.in";
        }
...
function check() {
    $.ajax({
        type: 'post',
        url: 'https://upi.cryptrx.in/order2/payment-status',
